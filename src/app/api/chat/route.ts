  ...
  try {
    ...
    if (mode === "generate-notes") {
      ...
      const result = await client.responses.create({
        ...
        store: false,
        max_output_tokens: 3000,
        instructions: `Write concise, useful speaker notes for EVERY slide in the approved slide plan below, one note per slide, in the same order as the slides. These notes are for the teacher's own spoken delivery — they are never shown to students and must never be treated as slide content.\n\nFor each slide, notes may include what the teacher should say, what to explain, what to emphasize, what to ask the class, and a useful transition into the next slide.\n\nDo not rewrite, reinterpret, or add to any slide's Title, Subtitle, Main Body Content, or Visual Idea — the slide plan is already approved; you are only producing notes to accompany it. Do not invent facts not supplied in the plan. Do not reveal information from a later slide while writing an earlier slide's notes — respect the teaching sequence exactly as given below.\n\nSlide 1 is the presentation's title slide and may show only a title and subtitle with little or no visible body content — its notes are not limited by that; they should explain how the teacher should introduce the lecture orally.\n\nKeep each slide's notes concise and directly usable while teaching.${globalRulesBlock}\nGENERAL PLAN:\n${JSON.stringify(contextFor(plan, "general"))}\nLECTURE SEQUENCE:\n${JSON.stringify(plan.timeline.map((r) => r.values))}\nAPPROVED SLIDE PLAN, IN ORDER:\n\n${fullSlidePlan(plan.slides)}\n\nReturn exactly one note per slide — ${plan.slides.length} notes total — in the same order as the slides above.`,
        input: [
          {
            role: "user" as const,
